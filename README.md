# 👋 Hi, I'm Avindu Lakshan Perera 🚀
### Software Engineer | Full-Stack Developer | BIT Undergraduate at University of Colombo

<p align="left">
  <img src="https://img.shields.io/badge/Role-Software%20Engineer-indigo?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloud-Oracle%20Cloud%20Infrastructure-F80000?style=for-the-badge&logo=oracle&logoColor=white" />
  <img src="https://img.shields.io/badge/Location-Sri%20Lanka-blue?style=for-the-badge&logoColor=white" />
</p>

---

### 💫 About Me
I am a versatile and driven **Software Engineer** specializing in Full-Stack development, currently pursuing a **Bachelor of Information Technology (BIT)** at the **University of Colombo** (2024 - 2027). I have a strong passion for architecting high-performance web applications using robust frontend frameworks and scalable backend architecture.

- 🎓 **Education:** Undergraduate at University of Colombo (BIT) | Industrial Diploma in Software Development (IDSD).
- 💻 **Core Tech Stack:** Specializing in Angular, TypeScript, Node.js, and Laravel.
- ☁️ **Cloud Exposure:** Deploying and maintaining digital architecture configurations over Oracle Cloud Infrastructure (OCI).
- 🚀 **Community Leadership:** Founder and leader of the **LAST HOPE** e-sports community—orchestrating competitive environments and driving technical workflows.

---

### 🛠️ Tech Stack & Ecosystem

<table>
  <tr>
    <td align="center" width="33%">
      🧑‍💻 <b>Frontend Development</b>
      <br/><br/>
      <img src="https://img.shields.io/badge/Angular-DD0031?style=flat-square&logo=angular&logoColor=white" alt="Angular" />
      <br/>
      <img src="https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
      <br/>
      <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript" />
      <br/>
      <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white" alt="TailwindCSS" />
    </td>
    <td align="center" width="33%">
      ⚙️ <b>Backend & Databases</b>
      <br/><br/>
      <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" alt="Laravel" />
      <br/>
      <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" alt="NodeJS" />
      <br/>
      <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white" alt="PHP" />
      <br/>
      <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" alt="MySQL" />
    </td>
    <td align="center" width="33%">
      ☁️ <b>Cloud, Devops & Multimedia</b>
      <br/><br/>
      <img src="https://img.shields.io/badge/Oracle_Cloud-F80000?style=flat-square&logo=oracle&logoColor=white" alt="OCI" />
      <br/>
      <img src="https://img.shields.io/badge/Git%20%26%20GitHub-181717?style=flat-square&logo=github&logoColor=white" alt="Git" />
      <br/>
      <img src="https://img.shields.io/badge/Vercel%20%26%20Netlify-000000?style=flat-square&logo=vercel&logoColor=white" alt="Hosting" />
      <br/>
      <img src="https://img.shields.io/badge/Photoshop-31A8FF?style=flat-square&logo=adobe-photoshop&logoColor=white" alt="Photoshop" />
    </td>
  </tr>
</table>

---

### 🏆 Featured Projects

* **🎮 [M&T E-sports Hub](https://mt-esports-frontend.vercel.app/)** - Secure frontend layout environment tailored for active team management. Dynamic views and analytics tracking built natively using **Angular & TypeScript** and hosted across **Oracle Cloud (OCI)**.
* **⚡ LAST HOPE Platform** - A customized tournament tracking platform featuring an automated bracket scoring engine built atop **Node.js** to handle match flow logic for 96 teams.
* **📊 Tournament Admin Panel** - A high-fidelity dashboard built with **Angular** to manage dense team data arrays, live point sheets, and dynamic standing updates.
* **🏦 E-Loan Management** - Full-stack requests and workflow schema implementation emphasizing role-based access control (RBAC), architected utilizing the **Laravel MVC** pattern.

---

### 📊 GitHub Stats & Performance Metrics

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=Avindu2003&show_icons=true&theme=tokyonight&count_private=true&hide_border=true" alt="Avindu's GitHub Stats" height="180px" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Avindu2003&layout=compact&theme=tokyonight&hide_border=true&langs_count=6" alt="Top Languages" height="180px" />
</p>

<p align="left">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Avindu2003&theme=tokyonight&hide_border=true" alt="GitHub Streak" height="180px" />
</p>

---

### 🤝 Connect with Me
- 💼 **LinkedIn:** [https://lk.linkedin.com/in/avindu-lakshan-b7b738327?trk=people-guest_people_search-card]
- 📧 **Email:** [avindu1lakshan1@gmail.com]
- 🌐 **Portfolio/Vercel:** [Your Portfolio Link Here]

<p align="right"><i>Keep coding, keep leveling up! 🚀</i></p>
